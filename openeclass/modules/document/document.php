...
if($is_adminOfCourse) {
	...
	Create new folder
	--------------------------------------*/
	$tool_content .= "\n      <li><a href=" . htmlspecialchars($_SERVER['PHP_SELF']) . "?createDir=".$cmdCurDirPath."'>$langCreateDir</a></li>";
        ...
        foreach (array(true, false) as $is_dir) {
                foreach ($fileinfo as $entry) {
                        ...
                        if ($is_dir) {
                                ...
                                $file_url = htmlspecialchars($_SERVER['PHP_SELF']) . "?openDir=$cmdDirName";
                                $link_extra = '';
                        ...
                        if ($is_adminOfCourse) {
                                /*** delete command ***/
                                $tool_content .= "\n    <td><a href=" . htmlspecialchars($_SERVER['PHP_SELF']) . "?delete=$cmdDirName' onClick=\"return confirmation('".addslashes($entry['filename'])."');\">";
                                $tool_content .= "<img src='../../template/classic/img/delete.gif' border='0' title='$langDelete' /></a>&nbsp;";
                                /*** copy command ***/
                                $tool_content .= "<a href=" . htmlspecialchars($_SERVER['PHP_SELF']) . "?move=$cmdDirName'>";
                                $tool_content .= "<img src='../../template/classic/img/move_doc.gif' border='0' title='$langMove' /></a>&nbsp;";
                                /*** rename command ***/
                                $tool_content .=  "<a href=" . htmlspecialchars($_SERVER['PHP_SELF']) . "?rename=$cmdDirName'>";
                                $tool_content .=  "<img src='../../template/classic/img/edit.gif' border='0' title='$langRename' /></a>&nbsp;";
                                /*** comment command ***/
                                $tool_content .= "<a href=" . htmlspecialchars($_SERVER['PHP_SELF']) . "?comment=$cmdDirName'>";
                                $tool_content .= "<img src='../../template/classic/img/information.gif' border='0' title='$langComment' /></a>&nbsp;";
                                /*** visibility command ***/
                                if ($entry['visible']) {
                                        $tool_content .= "<a href=" . htmlspecialchars($_SERVER['PHP_SELF']) . "?mkInvisibl=$cmdDirName'>";
                                        $tool_content .= "<img src='../../template/classic/img/visible.gif' border='0' title='$langVisible' /></a>";
                                } else {
                                        $tool_content .= "<a href=" . htmlspecialchars($_SERVER['PHP_SELF']) . "?mkVisibl=$cmdDirName'>";
