		...
		if(isset($URL)) {
			...
			if (($file_name != "") && ($file_size <= $diskQuotaVideo )) {
					...
					if (preg_match('/\.(ade|adp|bas|bat|chm|cmd|com|cpl|crt|exe|hlp|hta|' .'inf|ins|isp|jse|lnk|mdb|mde|msc|msi|msp|mst|pcd|pif|reg|scr|sct|shs|' .'shb|url|vbe|vbs|wsc|wsf|wsh)$/', $file_name)) {
						...
						$tool_content .= "<a href=\"" . htmlspecialchars($_SERVER['PHP_SELF']) . "\">$langBack</a></p><br />";
...
<form method=\"POST\" action=\"" . htmlspecialchars($_SERVER['PHP_SELF']) . "?submit=yes\" enctype=\"multipart/form-data\" id=\"edit_form\" onsubmit=\"return checkrequired(this, 'titre');\">
  ...
  <div id=\"operations_container\">
    <ul id=\"opslist\">
	  <li><a href=\"" . htmlspecialchars($_SERVER['PHP_SELF']) . "?form_input=file\">$langAddV</a></li>
      <li><a href=\"" . htmlspecialchars($_SERVER['PHP_SELF']) . "?form_input=url\">$langAddVideoLink</a></li>
    </ul>
  </div>";
	}
	...
	while ($myrow = mysql_fetch_array($result)) {
		switch($table){
			case "video":
				if(isset($vodServer)) {
					...
					$videoURL = "" . htmlspecialchars($_SERVER['PHP_SELF']) . "?action2=download&id=$myrow[1]'";
				}
		...
		if ($i%2) {
			...
			$tool_content .= "\n      <td align='center'><a href="
						...
						. "?id=$myrow[0]&table_edit=$table&action2=edit'><img src='../../template/classic/img/edit.gif' border='0' title='$langModify'></img></a>&nbsp;&nbsp;<a href="
						. htmlspecialchars($_SERVER['PHP_SELF'])
						. "?id=$myrow[0]&delete=yes&table=$table' onClick='return confirmation(\""
						.addslashes($myrow[2])
