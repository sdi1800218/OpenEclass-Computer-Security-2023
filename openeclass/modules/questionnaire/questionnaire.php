			...
			while ($theSurvey = mysql_fetch_array($active_surveys)) {
				...
				if (($visibility)||($is_adminOfCourse)) {
					...
					if ($is_adminOfCourse)   {
						...
						"<a href=" . htmlspecialchars($_SERVER['PHP_SELF'] . "?sdelete=yes&sid={$sid}' onClick='return confirmation();'>
						<img src='../../template/classic/img/delete.gif' border='0' title='$langDelete'></a>&nbsp;".
						"<a href=" . htmlspecialchars($_SERVER['PHP_SELF'] . "?visibility=$visibility_func&sid={$sid}'>
	...
	$k =0;
        ...
        <td>$theCreator</td>";
			...
			if ($is_adminOfCourse)  {
				...
				<a href=" . htmlspecialchars($_SERVER['PHP_SELF']) . "?delete=yes&pid=$pid' onClick='return confirmation();'>
				<img src='../../template/classic/img/delete.gif' title='$langDelete' border='0'></img></a>&nbsp;
				<a href=" . htmlspecialchars($_SERVER['PHP_SELF']) . "?visibility=$visibility_func&pid={$pid}'>
